...
char movehist[MAXMOVES] ; // 40
char solution[MAXMOVES] ; // 40
char top[M] ; // 48
double rube[20] ; // 160
char fmap[M][FACES] ; // 288
char map[M][NMOVES] ; // 864
char primary[12*12*12*12] ;
unsigned char *memp[12*12*12*12] ;
struct symfast symfast[MAXMOVES][POSITIONS] ; // 6720
char symmetrical[12*12*12*12] ;
char filename[100] ;
char *edge4_dist ;
char *corner4_dist ;
/**
 ...
 */
char edge_trans[NMOVES][CUBIES] ;
char corner_trans[NMOVES][CUBIES] ;
/**
 *   Calculate the above.
 */
 ...
 */
char edgerot[SYMMETRY][CUBIES] ;
char cornerrot[SYMMETRY][CUBIES] ;
char cubieperm[SYMMETRY][8] ;
/**
 *   The next, to the left, of the edges and corners, and also the
 *   flipped position, when rotating the whole cube.
 */
char edgeleft[20] = { 1, 3, 0, 2, 6, 4, 7, 5, 9, 11, 8, 10 } ;
char cornerleft[12] = { 2, 0, 3, 1, 6, 4, 7, 5 } ;
char edgeflip[20] = { 0, 2, 1, 3, 5, 4, 7, 6, 8, 10, 9, 11 } ;
char cornerflip[20] = { 1, 0, 3, 2, 5, 4, 7, 6 } ;
char flip3[3] = { 0, 2, 1 } ;
/**
...
void symmresort() {
   int i, j, f ;
   char safemap[M][NMOVES] ;
   char safefmap[M][FACES] ;
   char temptop[M] ;
 ...
 */
int genprim(int i, int j, int k, int m) {
...
static inline int lookupslow(struct symfast *p) {
   ...
   p->charoff = cc2 & 0xf ;
   return (*(p->p) >> p->charoff) & 15 ;
}
void lookupbump(struct symfast *p, int mul) {
   *(p->p) += mul * pwr[p->charoff >> 1] ;
}
int lookup(struct symfast *p) {
   int cf2 = corner_expand[0][p->c[4]] + corner_expand[1][p->c[5]] +
             corner_expand[2][p->c[6]] + corner_expand[3][p->c[7]] ;
   int cc2 = corner_compact2[cf2>>7] ;
   return (memp[p->ss>>4][(cc2 & ~0xf) + (p->ss & 0xf) +
                                    ((cf2 & 0x7f) << 4)] >> (cc2 & 0xf)) & 15 ;
}
/*
 *   Rotate and lookup.
 *
...
static inline int lookuprot(struct symfast *p) {
   ...
   return (memp[ss>>4][(cc2 & ~0xf) + (ss & 0xf) +
                                    ((ss2 & 0x7f) << 4)] >> (cc2 & 0xf)) & 15 ;
}
 ...
 */
/* double stats[10][10] ; */
int bumpuniq(struct symfast *ord, struct symfast *p, int mul) {
...
#ifdef HALFTURN
double xplorn(int dep, struct symfast *p, int depthleft, int prev) {
#else
double xplorn(int dep, struct symfast *p, int depthleft, int prev, int lastplus) {
#endif
   double r = 0.0 ;
   int f, i, j ;
   for (f=0; f<6; f++) {
      ...
      for (i=0; i<TWISTS; i++) {
         ...
         if (lookupslow(pp) != dep-depthleft+1)
            continue ;
         if (depthleft == 1) {
            r++ ;
            if (symmetrical[pp->ss>>4]) {
               ...
               plus += bumpuniq(pp, p+SYMMETRY, dep) ;
            } else {
               lookupbump(pp, dep) ;
            }
         } else {
...
#ifdef HALFTURN
            r += xplorn(dep, p+SYMMETRY, depthleft-1, f) ;
#else
            r += xplorn(dep, p+SYMMETRY, depthleft-1, f, i==0 && f != prev) ;
#endif
         }
         if (left == 0.0)
            return r ;
...
// } else {
#ifdef HALFTURN
      return xplorn(depthleft, p, depthleft, -100) ;
#else
      return xplorn(depthleft, p, depthleft, -100, 0) ;
#endif
// }
}
/**
...
char *getFileName() {
#ifdef HALFTURN
   sprintf(filename, "rub4h.dat") ;
#else
   sprintf(filename, "rub4q.dat") ;
#endif
   return filename ;
}
/**
...
unsigned char seenpats[TOTPATT][25] ;
unsigned char canonpat[25] ;
unsigned char *hashtab[HASHPRIME] ;
int npats = 0 ;
int botlev = 0 ;
unsigned char suff[SYMMMOVES] ;
int nsuff = 0 ;
int invmov(int m) {
