...
#ifdef HALFTURN
int peekahead(int left, int prev) {
#else
int peekahead(int left, int prev, int lastplus) {
#endif
   int i, j, f ;
   if (DEPTH >= left) {
   ...
   for (f=0; f<6; f++) {
      ...
      for (i=0; i<TWISTS; i++) {
         ...
         movehist[left] = m ;
#ifdef HALFTURN
         t = peekahead(left-1, f) ;
#else
         peekahead(left-1, f, i==0 && f != prev) ;
#endif
         if (bestval == 0)
            return 0 ;
#ifdef HALFTURN
